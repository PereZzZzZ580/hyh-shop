...
export default function CheckoutPage() {
  ...
  const [metodoPago, setMetodoPago] = useState<"WHATSAPP" | "COD" | "WOMPI">("WHATSAPP");
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (metodoEfectivo === "WOMPI") {
        const url = resp.wompi?.checkoutUrl;
        if (url) {
          window.location.assign(url);
          return;
        }
        // Fallback por si no vino URL: ir al pedido
        window.location.assign(`/pedido/${resp.orderId}`);
        return;
      }

      if (metodoEfectivo === "COD") {
        window.location.assign(`/pedido/${resp.orderId}`);
        return;
      }

      if (resp.whatsapp?.waLink) {
        window.location.assign(resp.whatsapp.waLink);
        return;
      }
      ...
      <form onSubmit={onSubmit} className="space-y-4">
        ...
        <div>
          ...
          <select
            value={metodoPago}
            onChange={(e) => {
              const val = e.target.value as "WHATSAPP" | "COD" | "WOMPI";
              if (val === "COD" && !esLocal) {
          ...
          >
            ...
            <option value="WOMPI">Tarjeta/Transferencia (Wompi)</option>

...
type DateRangeFilter = "today" | "3d" | "7d" | "all";
...
export default function PedidosPage() {
  ...
  const endpoint = `/api/orders?range=${range}`;
  const { data, error, isLoading, mutate } = useSWR<OrderListItem[]>(endpoint, fetcher);
  const isEmpty = !isLoading && !error && (data?.length ?? 0) === 0;
  const emptyMessage = range === "all" ? "No tienes pedidos aun." : "No hay pedidos en este rango.";
  ...
  return (
    <div className="container max-w-5xl pt-24 pb-16 text-yellow-100">
        ...
        {RANGE_OPTIONS.map((option) => {
          ...
          return (
            <button
              ...
              className={`h-9 rounded-lg px-4 border text-sm transition-colors ${active ? 'bg-yellow-400/20 border-yellow-300 text-yellow-100' : 'border-yellow-400/30 text-yellow-300 hover:bg-yellow-400/10'}`}
            >
              {option.label}
            </button>
          );
      ...
      {isEmpty && (
        <div className="rounded-xl border border-yellow-400/20 bg-black/40 p-6">{emptyMessage}</div>
      )}
